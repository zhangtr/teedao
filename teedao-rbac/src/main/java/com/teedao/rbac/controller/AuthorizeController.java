...
public class AuthorizeController {
        ...
        try {
            ...
            if(!subject.isAuthenticated()) {
                if(!login(subject, request)) {//登录失败时跳转到登陆页面
                    model.addAttribute("client", siteService.findByClientId(oauthRequest.getClientId()).getName() );
                    return "oauth2login";
                }
            }
            ...
            if (responseType.equals(ResponseType.CODE.toString())) {
                ...
                oAuthService.addAuthCode(authorizationCode, new OAuthCacheElement(username,cs.getSiteNo()));
            }
    ...
    private boolean login(Subject subject, HttpServletRequest request) {
        ...
        try {
            ...
            request.setAttribute("error", "失败次数过多，请稍候再试");
            return false;
        }catch (LockedAccountException e) {
            request.setAttribute("error", "账号冻结，请联系管理员");
            return false;
        }catch (Exception e) {
